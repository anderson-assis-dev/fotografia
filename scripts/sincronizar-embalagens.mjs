...
const SRC = path.join(process.cwd(), "fotos-embalagens");
const PUBLIC = path.join(process.cwd(), "public", "fotos-embalagens");
const FULL = path.join(PUBLIC, "full");
const THUMB = path.join(PUBLIC, "thumb");
const DATA = path.join(process.cwd(), "src", "data", "embalagens.js");
...
async function run() {
  if (!fs.existsSync(SRC)) {
    console.log("Pasta fotos-embalagens/ não encontrada.");
    return;
  }
  const arquivos = fs
    .readdirSync(SRC)
  ...
  for (let i = 0; i < arquivos.length; i++) {
    ...
    fotos.push(`/fotos-embalagens/full/${num}.jpg`);
    console.log(`  ${num}.jpg ← ${arquivos[i]}`);
  }
  const conteudo = `export const fotosEmbalagens = ${JSON.stringify(fotos, null, 2)};\n`;
  fs.writeFileSync(DATA, conteudo);
  console.log(`\n✓ ${fotos.length} embalagens em public/fotos-embalagens/ e src/data/embalagens.js`);
}
