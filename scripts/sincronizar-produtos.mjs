...
const SRC = path.join(process.cwd(), "FOTO DE PRODUTO - 2026");
const PUBLIC = path.join(process.cwd(), "public", "fotos-produtos");
const FULL = path.join(PUBLIC, "full");
const THUMB = path.join(PUBLIC, "thumb");
const DATA = path.join(process.cwd(), "src", "data", "produtos.js");
...
async function run() {
  if (!fs.existsSync(SRC)) {
    console.log("Pasta FOTO DE PRODUTO - 2026/ não encontrada.");
    return;
  }
  const arquivos = fs
    .readdirSync(SRC)
  ...
  for (let i = 0; i < arquivos.length; i++) {
    ...
    fotos.push(`/fotos-produtos/full/${num}.jpg`);
    console.log(`  ${num}.jpg ← ${arquivos[i]}`);
  }
  const conteudo = `export const fotosProdutos = ${JSON.stringify(fotos, null, 2)};\n`;
  fs.writeFileSync(DATA, conteudo);
  console.log(`\n✓ ${fotos.length} fotos em public/fotos-produtos/ e src/data/produtos.js`);
}
